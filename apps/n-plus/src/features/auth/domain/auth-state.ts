 ...
 *
 * Access token is NOT present here: this state passes through the React context and can be
 * read by any component. Tokens live in a private memory store.
 *
 * `unavailable` is intentionally separated from `unauthenticated`: backend not
 * responding is not proof that the user has logged out.
 */
 ...
 * destination can come from many reasons, and the transition table below is where
 * to decide which causes are acceptable.
 */
