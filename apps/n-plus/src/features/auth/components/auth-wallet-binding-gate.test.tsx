 ...
 *
 * A module-level variable assigned directly in `useEffect` will be overwritten by a previous test's
 * tree when a late response causes it to re-render after a subsequent test has already rendered —
 * the test inherits the closure of an unmounted component and incorrectly asserts on that tree.
 */
let activeGuardRef: GuardHolder = { current: null }

function assertReady(): void {
  if (activeGuardRef.current === null) {
    throw new Error("Guard not ready: call renderGate() first.")
  }
...
describe("no dismissal path", () => {
  ...
  it("shows both addresses so the user knows which wallet to switch to", async () => {
    ...
    // Query by the label the user hears, not by test id: if the label/value binding is broken,
    // the screen reader is also broken, and the test should fail for that exact reason.
    //
    // Matches the full rendered string, derived directly from `truncateAddress`. Asserting
    // the first 6 characters as before would stay green even if the tail displays the wrong
    // address — and a wrong tail is precisely the dangerous bug here.
