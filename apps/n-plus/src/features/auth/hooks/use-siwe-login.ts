...
export function useSiweLogin(): UseSiweLoginResult {
  ...
  // `useLayoutEffect`, not `useEffect`: passive effects are flushed after
  // paint and React may delay them under load. During that interval `selectionRef` still
  // holds the old selection, so a wallet change occurring mid-flow could slip through
  // `assertSnapshotStillValid`. Layout effects flush immediately after commit, closing
  // that window. This is also the pattern `useEvmWriteLifecycle` uses for stale-
  // operation refs.
  useLayoutEffect(() => {
  ...
  const signIn = useCallback(async (): Promise<SiweLoginResult> => {
    ...
    const result = await run<SiweLoginResult>(async () => {
      setError(null)
      ...
      try {
        ...
        const message = buildSiweMessage({
          domain: nonce.domain || snapshot.domain,
          address: snapshot.walletAddress,
          uri: nonce.uri || snapshot.uri,
          chainId: snapshot.chainId,
          nonce: nonce.nonce,
        ...
        if (payload.status === "registrationRequired") {
          assertSnapshotStillValid(snapshot)
