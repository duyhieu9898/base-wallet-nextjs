...
describe("refresh boundaries", () => {
  it("never auto-refreshes an auth protocol endpoint", async () => {
    ...
    for (const endpoint of Object.values(authProtocolEndpoints)) {
  ...
  it("auto-refreshes protected resources like /api/auth/profile on 401", async () => {
    tokenStore.set({ token: "expired-token", expiresAt: Date.now() + 1000 })
    ...
    const refresh = vi.fn().mockImplementation(() => {
      tokenStore.set({ token: "fresh-token", expiresAt: Date.now() + 1000 })
    ...
    await expect(client.request("/api/auth/profile")).resolves.toEqual({
      id: "user-1",
    })
