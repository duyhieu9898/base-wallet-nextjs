...
type ProbeHolder = { current: ProbeHandle | null }

/**
 * Each render writes to its own holder; the test only reads the active holder.
 *
 * A module-level binding assigned directly in `useEffect` will be overwritten by a previous test's
 * tree when a late response re-renders after a subsequent test has already rendered —
 * the test inherits the closure of an unmounted component and drives the wrong tree.
 */
let activeProbeRef: ProbeHolder = { current: null }

function currentProbe(): ProbeHandle {
  if (activeProbeRef.current === null) {
    throw new Error("Probe not ready: call renderLoggedIn() first.")
  }

  return activeProbeRef.current
}

const apiClientRef = {
  get current(): AuthenticatedApiClient {
    return currentProbe().apiClient
  },
}

function logoutRef(): Promise<void> {
  return currentProbe().logout()
}

function LogoutProbe({ holderRef }: { holderRef: ProbeHolder }) {
  const { state, logout, isLoggingOut, logoutError, apiClient } = useAuth()

  useEffect(() => {
    holderRef.current = { apiClient, logout }
  }, [holderRef, apiClient, logout])
...
async function renderAuthenticated() {
  ...
  const holderRef: ProbeHolder = { current: null }

  activeProbeRef = holderRef

  render(<LogoutProbe holderRef={holderRef} />, { wrapper: Wrapper })
...
describe("logout", () => {
  ...
  it("does not let a refresh that lands after logout restore the session", async () => {
    ...
    const pendingRequest = apiClientRef.current
