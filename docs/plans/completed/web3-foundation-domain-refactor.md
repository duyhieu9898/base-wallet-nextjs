...
- **không thay đổi behavior khi di chuyển code**, không làm yếu bất kỳ
  transaction safety invariant nào.
...
### Behavior changes có trong branch này

"Không đổi behavior" đúng với phần di chuyển code, nhưng branch không chỉ có di
chuyển. Hai thay đổi behavior được commit riêng, không trộn vào commit refactor:

- `5c2e7f9 fix(auth)` — `use-siwe-login.ts` đổi `useEffect` → `useLayoutEffect`
  cho `selectionRef`. Passive effect được flush sau paint và React có thể hoãn
  dưới tải; trong khoảng đó ref còn giữ selection cũ nên một wallet change giữa
  flow có thể lọt qua `assertSnapshotStillValid`. Auth nằm ngoài scope
  _refactor_, nhưng một correctness bug tìm thấy trên đường đi thì sửa, không
  để lại.
- `staking-approval-panel.tsx` — thêm control recovery sau khi approval revert
  (xem safety matrix). Đây là fix cho defect có sẵn từ trước refactor.

Ngoài ra `ff6ff36` và `4127dbe` chỉ sửa test auth (assertion chính xác hơn,
per-render holder thay module-level binding), không đổi production behavior.
...
`assertEvmWriteReady`, selection snapshot semantics, supported-chain behavior.
Không tạo generic `Web3Selection`.

Bản plan đầu liệt kê thêm `assertEvmReadReady`; symbol này chưa từng tồn tại
trong code (kiểm trên `9c619ee`, trước mọi commit refactor). Read hook gate bằng
`selection.status === "ready"` tại chỗ, không qua assert helper.
...
**Compatibility notes**: ESLint override tạm thời cho
`pending-receipt-reconciler` đã gỡ ở Phase 4 khi file về `transactions/history/`.

Một shim sót lại tới sau final audit: `evm/provider/config.ts`, re-export một
dòng `wagmiConfig` với lý do "giữ import path cũ cho `evm-provider.tsx`" — lý do
đó hết hiệu lực khi cả hai file về chung `provider/`. Đã xóa; `evm-provider.tsx`
import thẳng từ `./wagmi-config.adapter`.

**Import cycle qua barrel, phát hiện sau final audit.** Mitigation ở mục Risks
("domain `index.ts` không import root `index.ts`") được áp cho domain index
nhưng không cho component: 13 file trong `evm/**` — 9 component, 3 form và
`pending-receipt-reconciler` — import hook/type từ chính `@/web3/evm`, trong khi
`index.ts` lại export những file đó. Cycle thật, và nó tái lập đúng hazard
Phase 2 đã chẩn đoán: một file trong foundation kéo cả runtime graph vào chỉ để
lấy một type. Không gãy vì mọi binding chỉ dùng trong thân function, nhưng một
dòng dùng ở module scope là đủ để crash lúc import.

Cả 13 file giờ import từ module sở hữu symbol, giống ~80 file còn lại của
foundation. Barrel `@/web3/evm` chỉ còn là entrypoint cho application/feature,
không phải chỗ foundation tự đọc lại chính mình.
...
| stale operation                 | 0008       | `transactions/lifecycle/`                                | 12 test "stale …" (4 × 3 slice) + 1 ở staking               | pass   |
...
| feature write flow              | 0015       | `features/staking/hooks/use-staking-write.ts`            | `use-staking-write.test.tsx` (11)                           | pass   |
| feature write recovery          | 0015       | `features/staking/components/staking-approval-panel.tsx` | `staking-approval-panel.test.tsx` (5)                       | pass   |

Suite theo owner: lifecycle 19 · fees 9 · history 16 · invalidation 8 ·
native-transfer 36 · erc20-transfer 33 · erc20-approval 32 · staking write 16 ·
còn lại 122.

#### Hai hàng cuối được sửa sau final audit

Bản đầu ghi `feature write flow … pass` với evidence là `use-staking-write.test.ts`,
nhưng file đó chỉ có 2 test cho hàm thuần `isUsdcStakeAllowanceSufficient` —
không chạm prepare/confirm, lifecycle guard hay receipt. Đây đúng là thứ mục
"Safety" của Final Audit cấm: claim invariant mà không có test evidence. Nghiêm
trọng hơn vì kiến trúc đã **cố ý** đánh đổi ESLint lấy "test + review policy"
cho đúng ranh giới này (xem Decisions, 2026-08-05, mục ESLint/`0015`) — nên khi
test không tồn tại thì guarantee cũng không tồn tại.

`use-staking-write.test.tsx` thay thế file cũ và giữ lại 2 test preflight đó.
Năm invariant được mutation-test: bỏ duplicate-submit guard, bỏ
`isCurrentOperation`, bỏ `markReceiptHandled`, bỏ allowance preflight, và bỏ
nhánh `reverted` khỏi `receiptTerminal` — mỗi mutation làm đỏ đúng một test.

Hàng "feature write recovery" là hàng mới, ghi lại một defect UI mà refactor
không tạo ra nhưng cũng không phát hiện: khi approval receipt là `reverted`,
`review` và `hash` vẫn còn nên prepare, confirm và cả prepare staking đều bị
disable, mà panel không có nút `reset()`/`stopTrackingReceipt()` — người dùng
kẹt tới khi reload. Ba form của foundation (`token-approve-form` và hai form
transfer) đã có sẵn hai control này; panel staking giờ mirror chúng.
...
pnpm lint:         PASS
pnpm format:check: PASS
pnpm test:run:     PASS (54 files, 507 tests)
pnpm build:        PASS
pnpm web3:smoke:   not run
git diff --check:  PASS
```
...
Con số ở lần audit trước là 53 files / 493 tests. Chênh lệch: `-1` file và `-2`
test (`use-staking-write.test.ts` bị thay thế), `+2` file và `+16` test
(`use-staking-write.test.tsx` 11, `staking-approval-panel.test.tsx` 5).
...
- Test infra: `logout.test.tsx` đã chuyển sang per-render holder.
  `auth-runtime-provider.test.tsx` dùng biến cục bộ trong test nên không có race
  cùng loại. `auth-flow.integration.test.tsx` chỉ giữ selection store
  module-level, được reset ở `beforeEach`.
- `vi.mock()` trỏ path nội bộ: ESLint không thấy chúng vì không phải import
  statement. `src/test/mock-paths.test.ts` giờ assert mọi specifier `@/` trong
  `vi.mock` phải resolve tới file thật, nên một lần `git mv` bỏ sót sẽ đỏ ngay
  kèm tên file và specifier. Đã mutation-test.

  Không sửa được bằng cách "mock ở public boundary": `web3-lab.tsx` lấy **cả
  subject lẫn dependency từ cùng một barrel** — `BalanceCard`, `WalletCard`,
  `TransferSection` và `useEvmWallet` đều từ `@/web3/evm`; `StakingPositionCard`
  và `useStakingWrite` đều từ `@/features/staking`. `vi.mock` một barrel kèm
  factory sẽ thay luôn component đang được test. Muốn mock ở mức boundary thì
  phải tách barrel thành component vs hook — thay đổi kiến trúc, không phải sửa
  test. Nếu sau này cần, hướng đúng là một testing entrypoint được khai báo
  (`@/web3/evm/testing`), không phải nới ranh giới hiện tại.
