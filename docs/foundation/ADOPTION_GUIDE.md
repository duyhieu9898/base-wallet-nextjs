...
## Start from the supported runtime

The executable runtime is EVM. Keep its registry, wallet selection, read/write
guards, transaction lifecycle, typed errors, and provider composition when the
new application uses EVM.
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `src/features/auth/`                           | EVM EOA SIWE + MSW reference. It is not a production backend, RBAC model, or multi-wallet auth solution.                 |
| `src/features/staking/` and `contracts/`       | A small feature and contract fixture showing feature-local ownership. Replace it with the product contract or remove it. |
| `src/app/web3-lab/` and `src/components/web3/` | Development composition and reusable EVM UI examples. Keep, trim, or remove them deliberately.                           |
...
| `src/web3/chain-family-template/`              | Documentation for a future chain family only; it is not a runtime or SDK integration.                                    |

## Adding a real feature

Start feature-specific behavior under `src/features/<feature>/`. Own its
...
Do not promote feature code into `src/web3/` until at least two real consumers
...
[`../TESTNET_FUNDS.md`](../TESTNET_FUNDS.md); it requires test funds and must
never run in CI.
