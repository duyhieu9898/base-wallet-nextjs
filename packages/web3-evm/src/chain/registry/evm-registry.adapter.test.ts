...
describe("evm-registry.adapter", () => {
  ...
  it("Lookup always matches the real chain.id (no chain id stored in two places)", () => {
    ...
    // Every configured network is reachable by its own chain.id and nothing
    // else is: the derived map cannot drift from the list it came from.
    const reachable = getAllEvmNetworks().filter(
      (network) => findEvmNetworkByChainId(network.chain.id) === network,
    )
    expect(reachable).toHaveLength(getAllEvmNetworks().length)
  })
