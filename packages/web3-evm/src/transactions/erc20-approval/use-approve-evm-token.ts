...
import { findEvmToken } from "../../chain/registry/evm-registry.adapter"
import { type PreparedTokenApproval, prepareApproveEvmToken } from "./prepare"
import { buildTokenApprovalReview } from "./review"
import { createEvmWeb3Error, type EvmWeb3Error } from "../../errors/evm-errors"
import { useEvmFeeEstimate } from "../fees/use-evm-fee-estimate"
import { useEvmWriteLifecycle } from "../lifecycle/use-evm-write-lifecycle"
...
} from "../history/evm-transaction-history.storage"
import type { EvmTransactionReview } from "../review/evm-transaction-review"
import { deriveEvmWriteStatus } from "../lifecycle/evm-write-status"

export type UseApproveEvmTokenInput = {
  tokenAddress?: Address
  spenderAddress?: Address
  onReceiptSuccess?: () => void
}

export function useApproveEvmToken(input?: UseApproveEvmTokenInput) {
  const selection = useEvmSelection()
  const queryClient = useQueryClient()

  const tokenAddress = input?.tokenAddress
  const defaultSpender = input?.spenderAddress
  ...
  const [request, setRequest] = useState<PreparedTokenApproval | null>(null)
  ...
  const currentKey = `${chainId}:${account}:${tokenAddress}:${defaultSpender}`
  ...
  function prepare(prepareInput: { spender?: string; amount: string }) {
    ...
    const spender = prepareInput.spender ?? defaultSpender
    if (!spender) {
      throw createEvmWeb3Error(
        "INVALID_ADDRESS",
        "Spender address must be provided.",
      )
    }

    const prepared = prepareApproveEvmToken({
    ...
    const txReview = buildTokenApprovalReview({
      selection,
      prepared,
    })
  ...
  async function confirmApprove() {
    ...
    if (!simulated) {
      throw createEvmWeb3Error(
        "SIMULATION_FAILED",
        "Cannot approve: simulation result is not ready yet.",
      )
    }
      ...
      try {
        if (submittedTransaction) {
          addEvmTransactionHistoryItem({
            ...
            spender: submittedTransaction.spender,
          })
        }
      } catch {
        // Isolated storage side effect
