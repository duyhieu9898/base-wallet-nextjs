...
import { buildEvmWriteInvalidationFilters } from "./evm-invalidation.adapter"
...
describe("buildEvmWriteInvalidationFilters", () => {
  ...
  it("a token stake refreshes balance and allowance, because it spends both", () => {
    const { client, keys } = seedCache()
    const matched = matchedNames(
      client,
      keys,
  ...
  it("a native stake refreshes only the native balance", () => {
    const { client, keys } = seedCache()
