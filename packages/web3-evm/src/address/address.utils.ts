...
/**
 * Checks if a value is a 32-byte transaction hash (`0x` + 64 hex characters).
 */
 ...
 * Example: `0x3a4b91f8...f7a8b9c0`
 *
 * Lives beside `shortenAddress` on purpose: a table renders both identifiers the
 * same way, and `shortenAddress` returns "" for a 66-character hash because it
 * validates as an address first.
 */
export function shortenHash(
  ...
  if (!hash || !isValidTransactionHash(hash)) return ""
