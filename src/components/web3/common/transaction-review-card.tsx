...
export function TransactionReviewCard(props: {
  ...
  return (
    <div className="bg-muted/40 border-border space-y-3 rounded-lg border p-4">
      ...
      {review.isMainnet ? (
        <div className="flex items-center gap-1.5 rounded border border-red-500/30 bg-red-500/10 p-2 text-xs font-medium text-red-700 dark:text-red-400">
      ...
      {review.action === "token-approval" && review.isUnlimitedApproval ? (
        <div className="flex items-center gap-1.5 rounded border border-yellow-500/30 bg-yellow-500/10 p-2 text-xs font-medium text-yellow-700 dark:text-yellow-400">
          <AlertTriangle className="h-4 w-4 shrink-0" />
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        <div className="col-span-2">
          ...
          <span className="font-mono text-xs break-all">{review.account}</span>
        </div>
          ...
          {feeEstimate.status === "estimating" ? (
            ...
            <span className="text-destructive font-mono text-xs">
              {t.review.feeEstimateFailed.replace(
                "{message}",
                feeEstimate.error?.message ?? "RPC error",
              )}
            </span>
          ) : (
