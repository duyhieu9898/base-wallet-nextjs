...
import { useEvmAllowances } from "@/web3/evm/reads/allowances/use-evm-allowances"
...
describe("useEvmAllowances", () => {
  it("By default, owner = selected account and registry token list are used", () => {
    ...
    expect(lastParams.contracts?.[0].args).toEqual([ACCOUNT, SPENDER])
  })
  ...
  it("Distinguish between incorrectly formatted addresses and tokens that are not in the registry", () => {
    const { result } = renderHook(
      () =>
        useEvmAllowances({
          spenderAddress: SPENDER,
          tokenAddresses: [USDC_SEPOLIA, "0xnope" as Address, UNKNOWN_TOKEN],
        }),
      { wrapper },
    )
  ...
  it("Map the successful result and index it by canonical key", () => {
    queryState = {
      ...queryState,
      data: [{ status: "success", result: 5_000_000n }],
      isPending: false,
    }
    ...
    const key = `${CHAIN_ID}:${ACCOUNT.toLowerCase()}:${USDC_SEPOLIA.toLowerCase()}:${SPENDER.toLowerCase()}`
